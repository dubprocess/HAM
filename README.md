...
- **Local auth mode** — username/password login with no external IdP required
...
- **Login** — either a Fleet MDM + Okta setup, or use `LOCAL_AUTH=true` for simple username/password login with no external IdP

**Optional:**
- **Fleet MDM** — required for live device sync
- **Apple Business Manager** — adds procurement data and AppleCare warranty status
- **Slack** — enables alert notifications

> **No Okta?** Set `LOCAL_AUTH=true` in `backend/.env` and `REACT_APP_LOCAL_AUTH=true` in `frontend/.env.local`. HAM will create a default admin user on first start and show a username/password login form instead of the OIDC redirect.
>
> **No Fleet?** Jamf Pro support is stubbed and ready for contributors — see [docs/jamf.md](docs/jamf.md). Use `make seed` to populate demo data without any MDM.
...
**Option A — Local auth (no IdP required):**
...
LOCAL_AUTH_ADMIN_PASSWORD=changeme
```
...
Not connected to a live MDM yet? Populate the UI with realistic fake devices:

```bash
make seed
```

This creates ~50 demo assets covering all device types, statuses, and UI states. Run `make seed-clear` to wipe and reseed, or `make seed-wipe` to remove demo data before connecting your real MDM.
...
LOCAL_AUTH=false     # set true to skip OIDC entirely
```
...
| Local auth (no IdP) | ✅ Fully implemented | [docs/local-auth.md](docs/local-auth.md) |
...
- Mosyle, Iru, Intune — new `*_service.py` following the same interface as `fleet_service.py`
